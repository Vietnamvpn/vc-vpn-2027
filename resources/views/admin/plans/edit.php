...
<div style="margin-bottom: 0.2rem;">
    <h1 style="font-size: 1.6rem; font-weight: 700; letter-spacing: -0.5px; margin: 0; color: var(--ios-text);">Chỉnh Sửa Gói Cước: <?= htmlspecialchars($plan['name'] ?? '') ?></h1>
    <p style="font-size: 0.875rem; color: var(--ios-text-secondary); margin: 0.2rem 0 0 0;">Cập nhật lại thông số cấu hình và giá bán gói dịch vụ</p>
</div>

<hr style="border: none; border-top: 1px solid var(--glass-border); margin: 0.15rem 0 1.25rem 0;">

<div class="glass-card" style="padding: 1.75rem; width: 100%; border-radius: var(--radius-lg);">
    <form action="/admin/plans/edit?id=<?= $plan['id'] ?>" method="POST" style="display: flex; flex-direction: column; gap: 1.25rem;">
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.25rem;">
            <div>
                ...
                <input type="text" id="name" name="name" class="glass-input" value="<?= htmlspecialchars($plan['name'] ?? '') ?>" required autofocus style="width: 100%;">
            </div>
            ...
            <div>
                ...
                <input type="text" id="code" name="code" class="glass-input" value="<?= htmlspecialchars($plan['code'] ?? '') ?>" required style="width: 100%; text-transform: uppercase;">
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.25rem;">
                ...
                <select id="group_id" name="group_id" class="glass-input" required style="width: 100%; cursor: pointer;">
                    ...
                    <?php if (!empty($groups)): ?>
                        <?php foreach ($groups as $group): ?>
                            <option value="<?= $group['id'] ?>" <?= ($plan['group_id'] ?? 0) == $group['id'] ? 'selected' : '' ?>><?= htmlspecialchars($group['name']) ?></option>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="number" id="price" name="price" class="glass-input" value="<?= htmlspecialchars($plan['price'] ?? 0) ?>" min="0" step="1000" required style="width: 100%;">
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.25rem;">
            <div>
                ...
                <input type="number" id="duration_days" name="duration_days" class="glass-input" value="<?= htmlspecialchars($plan['duration_days'] ?? 30) ?>" min="1" required style="width: 100%;">
            </div>
            ...
            <div>
                ...
                <input type="number" id="bandwidth_limit_gb" name="bandwidth_limit_gb" class="glass-input" value="<?= htmlspecialchars($plan['bandwidth_limit_gb'] ?? 0) ?>" min="0" style="width: 100%;">
            </div>
            ...
            <div>
                ...
                <input type="number" id="max_devices" name="max_devices" class="glass-input" value="<?= htmlspecialchars($plan['max_devices'] ?? 1) ?>" min="1" required style="width: 100%;">
            </div>
        </div>
        ...
        <div>
            ...
            <select id="status" name="status" class="glass-input" style="width: 100%; cursor: pointer;">
                <option value="active" <?= ($plan['status'] ?? '') === 'active' ? 'selected' : '' ?>>Hoạt động (Active)</option>
                <option value="inactive" <?= ($plan['status'] ?? '') === 'inactive' ? 'selected' : '' ?>>Tắt (Inactive)</option>
            </select>
        </div>
        ...
        <div style="display: flex; justify-content: flex-end; margin-top: 0.5rem; gap: 0.75rem;">
            ...
            <button type="submit" class="glass-btn" style="padding: 0.65rem 1.75rem; font-size: 0.9rem; background: var(--ios-blue); color: #fff; border: none; cursor: pointer; border-radius: var(--radius-md); font-weight: 600;">💾 Cập Nhật Gói Cước</button>
        </div>
    </form>
</div>
